use crate::{engine::stack::{Stack, StackFrame}, loader::{Loader, runnable::Runnable}};
...
impl<'a> Runner<'a>
{
    ...
    pub fn run(&mut self) -> Result<(), RunnerError>
    {
        ...
        let initial_frame = self.stack.initial_frame(maxlocals, maxstack).ok_or(RunnerError::StackOverflow)?;
