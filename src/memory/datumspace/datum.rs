...
impl<'a> DatumPage<'a> {
    ...
    pub unsafe fn from_base_ptr(ptr: *const u8) -> Self {
        ...
        let id = unsafe { str::from_utf8_unchecked(
            std::slice::from_raw_parts(ptr.add(id_offset), header.id_len as usize)
        ) };

        let constants = unsafe {
            std::slice::from_raw_parts(
                ptr.add(const_offset) as *const Constant,
                header.constants_len as usize,
            )
        };

        let functions = unsafe {
            std::slice::from_raw_parts(
                ptr.add(fn_offset) as *const Runnable,
                header.functions_len as usize,
            )
        };

        DatumPage { id, constants, functions }
    }
}
